...
//                                            if (form.isValid()) {
                                            ...
                                            if(""!=url){
                                                Ext.Ajax.request({
                                                        url: path("/upload/validateCpdUploadCurrentOperation.json"),
                                                        method: "post",
                                                        dataType: "json",
                                                        success:function (data,act) {
                                                            ...
                                                            if(res.success){
                                                                form.submit({
                                                                    url:path('/upload/doUploadCpd.json'),
                                                                    //  waitMsg: '正在提交表单数据,请稍候...',
                                                                    method:'POST',
                                                                    success:function (action, data) {
                                                                        Ext.Msg.alert('提示信息', '文件成功上传');
                                                                        form.load({url:path("/operationHistory/lastedOperationLog.json?operationType=UPLOAD_CPD")});
//                                                                        form.findField("upload").setRawValue(url);
                                    ...
                                    },
                                    {
                                        ...
                                        handler:function () {
                                            Ext.Ajax.request({
                                                url:path("/upload/doDeleteCpdCurrentOperationByUser.json"),
                                                method:"post",
                                                dataType: "json",
                                                success:function (data,act) {
                                                    var res = Ext.decode(data.responseText);
                ...
                initComponent:function () {
                    var me = this;
                    me.callParent();
                    me.getForm().load({url:path("/operationHistory/lastedOperationLog.json?operationType=UPLOAD_CPD")});
                }
            });
        }
        ,
        ...
        create_form:function(){
            if (!this.upload_form) {
                ...
                this.upload_form = Ext.create('Upload.cpd.Panel', {
                    id:'uploadForm',
