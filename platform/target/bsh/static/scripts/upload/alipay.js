(function (scope) {
    ...
    var Initialize = {
         ...
         define:function () {
              ...
              * Date:2015/08/03
              */
            Ext.define('Upload.alipay.Panel', {
...
//                                            if (form.isValid()) {
                                            ...
                                            if(""!=url){
                                            Ext.Ajax.request({
                                                url: path("/upload/validateAlipayUploadCurrentOperation.json"),
                                                method: "post",
                                                dataType: "json",
...
//                                                                    form.load({url:path("/operationHistory/lastedOperationLog.json?operationType=UPLOAD_ALIPAY")});
//                                                                    form.findField("upload").setRawValue(url);
                                    ...
                                    },
                                    {
                                        ...
                                        handler:function () {
                                            Ext.Ajax.request({
                                                url:path("/upload/doDeleteAlipayCurrentOperationByUser.json"),
                                                method:"post",
                                                dataType: "json",
                                                success:function (data,act) {
                                                    var res = Ext.decode(data.responseText);
                ...
                initComponent:function () {
                    var me = this;
                    me.callParent();
                    me.getForm().load({url:path("/operationHistory/lastedOperationLog.json?operationType=UPLOAD_ALIPAY")});
                }
            });
        }
        ,
        ...
        create_form:function(){
            if (!this.upload_form) {
                ...
                this.upload_form = Ext.create('Upload.alipay.Panel', {
                    id:'uploadForm',
